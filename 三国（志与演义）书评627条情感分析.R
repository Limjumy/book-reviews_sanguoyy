...
reviewpath1 <- "E:/文本挖掘/三国/豆瓣书评/长评论/"
completepath1 <- list.files(reviewpath1, pattern = "*.txt$", full.names = TRUE)# 读取其绝对路径
reviewpath2 <- "E:/文本挖掘/三国/演义书评/演义长评论/"
completepath2 <- list.files(reviewpath2, pattern = "*.txt$", full.names = TRUE)

read.txt1 <- function(x) {
  des <- readLines(x)
  return(paste(des, collapse = ""))
}

read.txt2 <- function(x) {
  des <- readLines(x,encoding = "UTF-8")
  return(paste(des, collapse = ""))
}

reviews1 <- lapply(completepath1, read.txt1)
reviews2 <- lapply(completepath2, read.txt2)
length(reviews1) ##93
length(reviews2) ##534

reviews[1:93]<-reviews1
reviews[94:627]<-reviews2

reviews<-unlist(reviews)
reviews[1:4]

docname=0
docname1 <- list.files(reviewpath1, pattern = "*.txt$")# 读取文件名
docname2 <- list.files(reviewpath2, pattern = "*.txt$")
docname[1:93]<-docname1
docname[94:627]<-docname2
...
# 读标签id和label
d=read.csv("志+演义 书评等级.csv",header = F)
d=as.data.frame(d,col.names=F)

colnames(d) <- c("bianhao", "label")
d$id<-c(1:627)
d$label[d$label=="还行"]=3
d$label[d$label=="推荐"]=4
d$label[d$label=="力荐"]=5
...
trainterm$id2<-c(1:nrow(trainterm))
trainterm$id2[trainterm$term== ""]
write.csv(trainterm,"trainterm(yy+z).csv",row.names = F)
trainterm$term[trainterm$id2==15713]
...
#write.csv(traintfidf,"traintfidf_sanguo(yy+z)_shuping.csv",row.names = F)
